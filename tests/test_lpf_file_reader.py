...
def test_lpf_file_reader_mono_v1():
    ...
    assert raypaths[0].impacts[1] == Point3f(x=4.4084320068359375, y=14.999999046325684, z=2.4493408203125)
    ...
    assert raypaths[0].lastDirection == Point3f(x=0.2041478008031845, y=-0.9723469614982605, z=0.11342425644397736)

    expectedNbOfImpactOnFaces9 = 6
    assert len(raypaths[9].impacts) == expectedNbOfImpactOnFaces9
    assert raypaths[9].impacts[1] == Point3f(x=-1.8186546564102173, y=15.0, z=-6.767658233642578)
    ...
    assert raypaths[9].lastDirection == Point3f(x=0.14110437035560608, y=0.8392737507820129, z=0.5250800848007202)
...
def test_lpf_file_reader_multi_v1():
    ...
    assert raypaths2[3].impacts[1] == Point3f(x=5.026374340057373, y=15.000000953674316, z=0.7341787815093994)
    ...
    assert raypaths2[3].lastDirection == Point3f(x=0.09542781859636307, y=0.9953387975692749, z=0.013935667462646961)
